...
class ZhipuProvider:
    ...
    def chat(
        ...
        **kwargs
        ...
        """Send a chat request to Zhipu API."""
        
        # Default Zhipu chat parameters
        chat_params = {
            "model": model,
            "messages": messages,
        }
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # Convert response to Ollama-like format for compatibility
                ...
                if is_rate_limit and attempt < MAX_RETRIES - 1:
                    ...
                    print(
                        f"[ZhipuProvider] Rate limit hit "
                        f"(attempt {attempt + 1}/{MAX_RETRIES}). "
                        f"Retrying in {sleep_time}s..."
                    )
                    time.sleep(sleep_time)
...
class GroqProvider:
    """Groq API provider implementation using OpenAI compatible endpoint."""
