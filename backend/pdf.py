...
class PDFHandler:
    ...
    def _extract_all_sections_separately(
        ...
        # Helper to fetch section data
        def fetch_section(sec_name):
            data = self._extract_section_data(text_content, sec_name)
            return sec_name, data

        # Run concurrently
        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
            future_to_section = {executor.submit(fetch_section, sec_name): sec_name for sec_name in sections}
            ...
            for future in concurrent.futures.as_completed(future_to_section):
                ...
                try:
                    ...
                    logger.error(f"⚠️ {sec_name} generated an exception: {exc}")
                    return None
